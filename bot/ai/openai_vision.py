...
3. Если изображение нечёткое, блюдо не опознано или порция неясна — выставь confidence="low" и needs_clarification=true.
4. Никогда не выдумывай точные цифры при неопределённости. Лучше спроси у пользователя.
5. confidence="high" только если блюдо чётко распознано И видна порция.
6. confidence="medium" если блюдо понятно, но порция приблизительная.
7. Напитки включай только если они явно видны на фото.
8. Все текстовые поля — на русском языке.
9. Если на фото нет еды вообще — needs_clarification=true, clarification_prompt="На фото не видно еды. Отправь другое фото или опиши приём пищи текстом."
"""
